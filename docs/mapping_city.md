...
**Source:** [NoahAlting/CFTree](https://github.com/NoahAlting/CFTree). External preprocessor (Linux / WSL conda env) writes one CityJSON 2.0 tile per AHN sub-tile, assigning each tree to one owning tile so the tiles hold no cross-tile duplicates; [`tools/merge_cftree_tiles.py`](../tools/merge_cftree_tiles.py) merges and clips those to the boundary into the single [`inputs/vegetation/*.city.json`](../inputs/vegetation/) the loader reads (see [`inputs/vegetation/README.md`](../inputs/vegetation/README.md)).
