...
| `b3_h_maaiveld` | `0.175` | ✓ | (a) feeds into `nrg3:bdgHeight` as the subtrahend in `b3_h_dak_max - b3_h_maaiveld` (`_apply_building_attributes`); (b) rewrites every LoD 0 footprint vertex's Z from 3DBAG's nominal `0.001 m` to this terrain height, so the LoD 0 polygon sits co-planar with the LoD 1 ground face (see § 5b LoD 0 row); (c) the envelope computation mirrors the same lift (`pand_executor._collect_coordinates`) so `gml:Envelope` Z extent matches the emitted vertices. | `_apply_building_attributes`, `build_building`, `_collect_coordinates`. |
...
**Matcher:** [`tree_matching.match_nearest_within`](../citygml_energy/city_builder/tree_matching.py), called inline from `pipeline._maybe_match_trees_to_bgt` with `register_label="BGT boom"` and `radius_m=tree_matching.MATCH_RADIUS_M` — nearest-neighbour against CFTree crown centroids at a 4 m radius.
...
**Matcher:** [`tree_matching.match_nearest_within`](../citygml_energy/city_builder/tree_matching.py), called inline from `pipeline._maybe_match_trees_to_bor` with `register_label="Emmen BOR"` and `radius_m=tree_matching.MATCH_RADIUS_M` — nearest-neighbour against CFTree crown centroids at a 4 m radius.
...
|---|---|
| `gml:Envelope` on the `CityModel` | union of every emitted vertex: per-Pand building LoD vertices restricted to the LoDs the user requested via `config.lods` (`_collect_coordinates` in [`pand_executor.py`](../citygml_energy/city_builder/pand_executor.py); LoD 0 footprint Z is mirrored from 3DBAG's nominal 0.001 m to `b3_h_maaiveld` to match the build-time lift in the building builder), every projected solar-panel vertex (`_collect_solar_panel_coordinates`), every BAG VBO address point at Z=0 (`_collect_address_coordinates`; BAG `geometriePunt` is 2D, the multiPoint builder pads Z to 0.0), and every tree crown vertex (CFTree). Written last so the envelope bounds everything that went into the file. Restricting building vertices to the requested LoDs prevents an `lods=[0]` run from claiming the LoD 2.2 mesh's roof-tip Z extent. |
...
| `m3/a` | `nrg3:Energy/amount` on the CBS Postcode6 `naturalGas` resource for each `nrg3:UrbanFunctionArea` | `UOM name="CUBIC_METRE_PER_YEAR" id="m3/a"` (UOMList v1.2, TU Delft proposal, pending upstream FZK merge). UOMList had `m3/s` / `m3/h` flow-rate forms but no annual-volume token. | primary id ✓ (local) |
| `kWh/a` | `nrg3:Energy/amount` on the CBS Postcode6 `electricity` resource for each `nrg3:UrbanFunctionArea` | `UOM name="KILOWATT_HOUR_PER_YEAR" id="kWh/a"` (UOMList v1.2, TU Delft proposal, pending upstream FZK merge). UOMList had `kWh` (no per-time) and `MWh/a` (which would force a 1000× rescale of the CBS-published values). | primary id ✓ (local) |

The six `*/a` tokens are NL convention (NTA 8800 reports BENG metrics in kWh/m²·jaar; the legacy NEN 7120 lineage reports its totals in MJ/jaar and kg/jaar; CBS Postcode6 reports per-dwelling annual averages in m³/jaar and kWh/jaar). The first four (`kWh/m2/a`, `kg/m2/a`, `MJ/a`, `kg/a`) shipped upstream in FZK UOMList v1.1 (2026-05-05) and resolve cleanly in every consuming viewer; the two CBS-specific tokens (`m3/a`, `kWh/a`) were added locally in UOMList v1.2 (2026-05-27) as TU Delft–proposed extensions, pending upstream FZK merge. A future viewer that loads only the FZK upstream `UOMList.xml` (without this project's local v1.2 superset) would display the two CBS tokens verbatim until the upstream catches up.
...
6. **FZKViewer UOMList previously lacked the NL energy tokens; mostly closed in v1.1, with two remaining gaps.** `kWh/m2/a`, `kg/m2/a`, `MJ/a`, and `kg/a` shipped upstream in FZK UOMList v1.1 (2026-05-05) and now resolve cleanly. `m3/a` and `kWh/a` (CBS Postcode6 per-dwelling annual averages) and `W/W` (PV efficiency, heat-pump COP) were added in the bundled UOMList v1.2 (2026-05-27) as TU Delft–proposed extensions, pending upstream FZK merge. See Appendix B for the per-token status and the bundled [`UOMList.xml`](../KITModelViewer_V7.5.2_Build-3777/Data/UOMList.xml) for the in-line rationale.
