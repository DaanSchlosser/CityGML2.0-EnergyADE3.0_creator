...
**Source:** [NoahAlting/CFTree](https://github.com/NoahAlting/CFTree). External preprocessor (Linux / WSL conda env) writes one CityJSON 2.0 tile per AHN sub-tile; [`tools/merge_cftree_tiles.py`](../tools/merge_cftree_tiles.py) merges, clips to the boundary, and dedupes those into the single [`inputs/vegetation/*.city.json`](../inputs/vegetation/) the loader reads (see [`inputs/vegetation/README.md`](../inputs/vegetation/README.md)).
...
| presence in BGT | - | ✓ | presence of the `core:externalReference` element | A CFTree tree matched to a BGT record is a **municipally-maintained (public-space) tree**; an unmatched CFTree reconstruction is almost certainly a private garden tree. Encoded via the presence / absence of the `externalReference`. On the Emmer-Compascuum small-area run: 218 of 702 CFTree trees are BGT-matched (~31 %). |
...
- **Energy fields** (`gemiddeldGasverbruikWoning`, `gemiddeldElektriciteitsverbruikWoning`) use `_int_preserve_sentinel`, which preserves the raw CBS sentinel verbatim (only `null` / empty folds to `None`). The sentinel rides into `nrg3:Energy/amount` literally so the downstream consumer can distinguish the three suppression reasons (`-99997` privacy, `-99995` deferred, `-99999` unknown) from a real positive measurement by reading the amount itself. This trades physical plausibility (negative consumption is meaningless) for full source fidelity (every CBS row round-trips). The `nrg3:Metadata/qualityDescription` co-located on the area documents the sentinel meanings so the encoding is self-describing.
...
| `kWh/m2/a` | `nrg3:Energy/amount` on the four NTA 8800 EP-Online resources, `nrg3:EnergyPerformanceCertificate/value` on NTA 8800 certs | `UOM name="KILOWATT_PER_SQUAREMETER_PER_YEAR" id="kWh/m2/a"` (UOMList v1.1) | primary id ✓ |
| `kg/m2/a` | `nrg3:Energy/co2Equivalent` on the BENG-2 resource for NTA 8800 rows | `UOM name="KILOGRAM_PER_SQUAREMETER_PER_YEAR" id="kg/m2/a"` (UOMList v1.1) | primary id ✓ |
| `MJ/a` | `nrg3:Energy/amount` on the single legacy `primary` resource for legacy_total rows, `nrg3:EnergyPerformanceCertificate/value` on legacy_total certs | `UOM name="MEGAJOULE_PER_YEAR" id="MJ/a"` (UOMList v1.1). Required by the regime asymmetry: legacy methods report an absolute MJ/yr total, not a per-m² intensity. | primary id ✓ |
| `kg/a` | `nrg3:Energy/co2Equivalent` on the legacy `primary` resource for the Nader Voorschrift / ISSO branch | `UOM name="KILOGRAM_PER_YEAR" id="kg/a"` (UOMList v1.1). Companion to `MJ/a`. | primary id ✓ |
| `m3/a` | `nrg3:Energy/amount` on the CBS Postcode6 `naturalGas` resource for each `nrg3:UrbanFunctionArea` | `UOM name="CUBIC_METRE_PER_YEAR" id="m3/a"` (UOMList v1.2, TU Delft proposal, pending upstream KIT merge). UOMList had `m3/s` / `m3/h` flow-rate forms but no annual-volume token. | primary id ✓ (local) |
| `kWh/a` | `nrg3:Energy/amount` on the CBS Postcode6 `electricity` resource for each `nrg3:UrbanFunctionArea` | `UOM name="KILOWATT_HOUR_PER_YEAR" id="kWh/a"` (UOMList v1.2, TU Delft proposal, pending upstream KIT merge). UOMList had `kWh` (no per-time) and `MWh/a` (which would force a 1000× rescale of the CBS-published values). | primary id ✓ (local) |

The six `*/a` tokens are NL convention (NTA 8800 reports BENG metrics in kWh/m²·jaar; the legacy NEN 7120 lineage reports its totals in MJ/jaar and kg/jaar; CBS Postcode6 reports per-dwelling annual averages in m³/jaar and kWh/jaar). The first four (`kWh/m2/a`, `kg/m2/a`, `MJ/a`, `kg/a`) shipped upstream in KIT UOMList v1.1 (2026-05-05) and resolve cleanly in every consuming viewer; the two CBS-specific tokens (`m3/a`, `kWh/a`) were added locally in UOMList v1.2 (2026-05-27) as TU Delft-proposed extensions, pending upstream KIT merge. A future viewer that loads only the KIT upstream `UOMList.xml` (without this project's local v1.2 superset) would display the two CBS tokens verbatim until the upstream catches up.
