...
| **Cross-reference** | `core:externalReference` element with `informationSystem` (catalogue page) plus `externalObject/uri` (dereferenceable feature URL). Used to link our output back to the authoritative source, not to ship attributes. |
...
| 5 | 3DBAG CityJSON tile | 62 `Building` attributes + LoD0/1.2/2.2 geometries | 8 attributes + 3 LoD geometries | `bldg:lod0FootPrint` + `bldg:lod1Solid` + `bldg:boundedBy/lod2MultiSurface` + `nrg3:bdgHeight` (`QualifiedHeight`, `type="maxHeightAboveGround"`) + `bldg:roofType` + `bldg:storeysAboveGround` + `nrg3:bdgVolume` |
...
| 8 | CFTree `trees_lod3.city.json` | 10 attributes + LoD3 geometry | 9 + geometry | `veg:SolitaryVegetationObject` + `veg:lod3Geometry` |
...
| `gebruiksdoel[0]` | `"woonfunctie"` | ✓ | `nrg3:type` (`gml:CodeType`) with `codeSpace=CS_BAG_GEBRUIKSDOEL`. Kept verbatim as the Dutch term rather than mapped to the Energy ADE `CurrentUseValue.xml` codelist (which would lose distinctions like `logiesfunctie` vs `woonfunctie`). | Additional values in a dual-use VBO are dropped. |
...
| (NL country wrapper) | - | ✓ | `xAL:Country/xAL:CountryNameCode Scheme="iso.3166-1 alpha-2"`=`NL` + `xAL:CountryName="The Netherlands"`. | Mirrors the Energy ADE 3.0 Alderaan reference shape. |
...
## 5. 3DBAG CityJSON tile: LoD0 / 1.2 / 2.2 geometries + building attributes
...
| `b3_h_maaiveld` | `0.175` | ✓ | (a) feeds into `nrg3:bdgHeight` as the subtrahend in `b3_h_dak_max - b3_h_maaiveld` (`_apply_building_attributes`); (b) rewrites every LoD0 footprint vertex's Z from 3DBAG's nominal `0.001 m` to this terrain height, so the LoD0 polygon sits co-planar with the LoD1 ground face (see § 5b LoD0 row); (c) the envelope computation mirrors the same lift (`pand_executor._collect_coordinates`) so `gml:Envelope` Z extent matches the emitted vertices. | `_apply_building_attributes`, `build_building`, `_collect_coordinates`. |
...
|---|---|---|---|---|
| `"0"` | MultiSurface (one polygon) | ✓ | `bldg:lod0FootPrint`, with each footprint vertex's Z rewritten from 3DBAG's nominal `0.001 m` to the per-Pand terrain height `b3_h_maaiveld` (so the LoD0 polygon sits co-planar with the LoD1 ground face rather than floating at NAP=0). The Z rewrite is skipped when `b3_h_maaiveld` is absent from the 3DBAG attributes (source-Z is preserved as a fallback). | `build_building`. The envelope computation in [`pand_executor._collect_coordinates`](../citygml_energy/city_builder/pand_executor.py) mirrors the same lift so the `gml:Envelope` Z extent matches what is actually written into the output. |
| `"1.2"` | Solid with thematic `semantics` | ✓ | `bldg:lod1Solid` as a `gml:CompositeSurface` shell | `build_building` |
| `"2.2"` | Solid with per-face `GroundSurface` / `WallSurface` / `RoofSurface` semantics | ✓ | `bldg:boundedBy` → `bldg:GroundSurface` / `WallSurface` / `RoofSurface`, **one element per polygon** with a single-polygon `bldg:lod2MultiSurface`. Each surface also carries `nrg3:bdgBdrySurfTotalSurfaceArea` (m², holes subtracted), `nrg3:bdgBdrySurfInclination` (deg, [0, 180] from +Z, so a flat roof is 0, a wall is 90, a downward-facing ground slab is 180), and `nrg3:bdgBdrySurfAzimuth` (deg, compass bearing in `[0, 360)`, omitted on horizontal surfaces; the bearing is `round`-then-`% 360.0` so a ~359.998 ° face does not emit `360.0` and break the half-open contract). | `_attach_lod2_thematic_surfaces`, `_attach_planar_surface_ade_attributes`, sliver-area gate `thematic_surface_attrs`. Sliver-area filter: facets whose `planar_surface_attributes` is `None` (sub-nm² Newell magnitude) or whose net area falls below `MIN_FACE_AREA_M2 = 1e-3` (3DBAG LoD2.2 mesh-sliver wall artefact, see [`docs/threedbag_sliver_walls.md`](threedbag_sliver_walls.md)) are dropped before `bldg:boundedBy` is appended, so no zero-area surface enters the output. The same gate is consulted by the solar-panel matcher in [`solar_panels._collect_roof_facets`](../citygml_energy/city_builder/solar_panels.py), keeping the two emit-set predicates in sync: a panel cannot end up `installedOn` a sliver-area roof facet that `_attach_lod2_thematic_surfaces` never emitted. Construction / radiation properties (Thickness, HeatCapacity, IsShared, view factors, OpaqueSurfaceArea, ThermalBridgeUValue) are deliberately absent in this pipeline because the city pipeline has no source for them; populating with placeholders would silently contaminate downstream energy analyses. The per-building pipeline, which *does* have layered constructions, additionally fills `nrg3:bdgBdrySurfThickness` and `nrg3:bdgBdrySurfHeatCapacity` from the LayeredConstruction layer stack (see [`mapping_building.md`](mapping_building.md) and [`citygml_energy/boundary_attributes.py`](../citygml_energy/boundary_attributes.py)). |
...
|---|---|---|---|
| `nrg3:EnergyPerformanceCertificate` | line 1335 | `[0..*]` natively on BuildingUnit (XSD line 1438, composition slot on `BuildingUnitType`); also `[0..*]` on Building via the `nrg3:energyPerformanceCertificate` substitution element (XSD line 1538). The pipeline uses the BuildingUnit slot because every EP-Online cert is per-VBO. | Slots: `type`, `label`, optional `value` (`gml:MeasureType`), optional `certificationMethod`; inherited `validFrom` / `validTo`. |
| `nrg3:Energy` (extends `AbstractResource`) | line 2044 | child of any `AbstractCityObject` via the `nrg3:resource` substitution element (line 1277) | One per (energy-type, end-use) pair. Slots: `type` (Code, e.g. `net` / `primary` / `final`), `endUse` (Code, e.g. `spaceHeating` / `spaceCooling` / `domesticHotWater`), `energyCarrier`, `source`. Inherited from `AbstractResource` (line 544): `amount` (Measure), `referencePeriod` (Code, e.g. `"year"`), `isAmountNormalized` (boolean, required), `normalizationValue` (Measure, optional), `normalizationParameter` (string, optional, e.g. `"netFloorArea"`), `co2Equivalent` (Measure, optional). |
...
| `nrg3:bdgType` (extension on `_AbstractBuilding`) | line 1510 | XSD `[0..*]` on Building, but the UML `taggedValue` annotation on the element declares `maxOccurs=1`, so the conceptual cardinality is `[0..1]`; the pipeline emits exactly one canonical Pand-level value. | `gml:CodeType`. The `@codeSpace` attribute identifies the vocabulary; this pipeline uses the Dutch RVO NTA 8800 typology verbatim with `@codeSpace = CS_RVO_GEBOUWTYPE`. |
...
1. **`nrg3:Energy` attaches directly to any `AbstractCityObject` via the `nrg3:resource` substitution element** (line 1277: `<element name="resource" substitutionGroup="core:_GenericApplicationPropertyOfCityObject" type="nrg3:AbstractResourcePropertyType">`). Building, BuildingUnit, Zone, Device, all of them host an `nrg3:resource` slot. EP-Online metrics are VBO-scoped, so they parent under the BuildingUnit. No xlink, no top-level workaround.
...
- The thermal-zone area (`GebruiksoppervlakteThermischeZone`) is emitted as a second `nrg3:QualifiedArea` on each BuildingUnit, alongside the BAG `oppervlakte` entry. `BuildingUnit/area` is `[0..*]`.
...
<bldg:Building gml:id="pand_0114100000206140">
  ...
  <!-- ...remaining 3DBAG-derived bldg:* attributes (bldg:roofType,
       bldg:storeysAboveGround) and LoD0/1/2 geometries elided for
  ...
  <nrg3:buildingUnit>
    <nrg3:BuildingUnit gml:id="bu_0114010000274521">
      ...
      <!-- nrg3:address on the BuildingUnit is an xlink reference: the
           ...
           relationType="association" (XSD line 1431-1437), so the
      ...
      <nrg3:energyPerformanceCertificate>
        <nrg3:EnergyPerformanceCertificate gml:id="epc_0114010000274521">
          ...
          <!-- BENG-3 renewable-energy share. No native Energy ADE slot, so it
               uses the gen: ADE hook on the EPC itself: beta8 re-rooted the
...
| `geom` | MULTIPOLYGON (EPSG:28992) | ✓ | Projected onto the matched LoD2 roof plane to form the collector's `lod2MultiSurface`. |
...
|---|---|---|
| `nrg3:moduleArea` | on-roof (tilted) panel area: the 2D footprint divided by `cos(inclination)`, equal to the area of the emitted `lod2MultiSurface` (coincides with the flat footprint only on a horizontal roof) | `m2` |
...
## 8. CFTree `trees_lod3.city.json`: LoD3 tree reconstructions from AHN LiDAR

**Source:** [NoahAlting/CFTree](https://github.com/NoahAlting/CFTree), run from the [DaanSchlosser/CFTree](https://github.com/DaanSchlosser/CFTree) fork. External preprocessor (the prebuilt docker image by default, or a Linux / WSL conda env) writes one CityJSON 2.0 tile per AHN sub-tile, assigning each tree to one owning tile so the tiles hold no cross-tile duplicates; [`tools/merge_cftree_tiles.py`](../tools/merge_cftree_tiles.py) merges and clips those to the boundary into the single [`inputs/vegetation/*.city.json`](../inputs/vegetation/) the loader reads (see [`inputs/vegetation/README.md`](../inputs/vegetation/README.md)).
...
**Appearance:** only each tree's `gml:MultiSurface` container id is collected during attachment and passed to `append_vegetation_appearance`; the colour propagates to its member polygons per the CityGML 2.0 Appearance model, so the member `gml:Polygon` ids are deliberately not targeted (matching the Alderaan reference, whose targets list only containers, and § 5b / Appendix A). The painter emits an `app:Appearance` with theme `"vegetation"` and `diffuseColor = 0.15 0.55 0.15` (deep foliage green).
...
**Matcher:** [`tree_matching.match_nearest_within`](../citygml_energy/city_builder/tree_matching.py), called from [`vegetation._match_to_bgt`](../citygml_energy/city_builder/vegetation.py) (dispatched concurrently with the BOR match by `_match_to_registers`) with `register_label="BGT boom"` and `radius_m=tree_matching.MATCH_RADIUS_M`: nearest-neighbour against CFTree crown centroids at a 4 m radius.
...
| presence in BGT | - | ✓ | presence of the `core:externalReference` element | A CFTree tree matched to a BGT record is a **municipally-maintained (public-space) tree**; an unmatched CFTree reconstruction is almost certainly a private garden tree. Encoded via the presence / absence of the `externalReference`. On the Emmer-Compascuum small-area run: 219 of 702 CFTree trees are BGT-matched (~31 %). |
...
**Matcher:** [`tree_matching.match_nearest_within`](../citygml_energy/city_builder/tree_matching.py), called from [`vegetation._match_to_bor`](../citygml_energy/city_builder/vegetation.py) (dispatched concurrently with the BGT match by `_match_to_registers`) with `register_label="Emmen BOR"` and `radius_m=tree_matching.MATCH_RADIUS_M`: nearest-neighbour against CFTree crown centroids at a 4 m radius.
...
The two coercion helpers in [`fetchers/cbs_postcode6.py`](../citygml_energy/city_builder/fetchers/cbs_postcode6.py) treat the sentinel block asymmetrically by field type:

- **Energy fields** (`gemiddeldGasverbruikWoning`, `gemiddeldElektriciteitsverbruikWoning`) use `_int_preserve_sentinel`, which preserves the raw CBS sentinel verbatim (only `null` / empty maps to `None`). The sentinel is written into `nrg3:Energy/amount` literally so the downstream consumer can distinguish the three suppression reasons (`-99997` privacy, `-99995` deferred, `-99999` unknown) from a real positive measurement by reading the amount itself. This trades physical plausibility (negative consumption is meaningless) for full source fidelity (every CBS row round-trips). The `nrg3:Metadata/qualityDescription` co-located on the area documents the sentinel meanings so the encoding is self-describing.
- **Dwelling-count fields** (`aantalWoningen`, `aantalNietBewoondeWoningen`) keep the older `_value_or_suppressed` path, which folds *any* value at-or-below `-99000` to `None`. A negative `gen:intAttribute name="dwellingCount"` would be physically incoherent; counts are total dwellings registered in BAG and cannot be negative by construction.

**Publication cadence note.** CBS publishes geometry + dwelling counts immediately for each vintage but releases energy values (gemiddeldGasverbruikWoning, gemiddeldElektriciteitsverbruikWoning) on a one-year lag (CBS Longread §4: *"De jaarbestanden bevatten per jaar onderwerpen voor zover deze beschikbaar zijn rond de jaarwisseling. In een volgende versie zullen jaarbestanden worden aangevuld met dan beschikbaar gestelde gegevens"*). Empirical state at time of writing (2026-05): vintages 2022 and 2023 ship populated energy fields nationwide; the 2024 vintage ships every energy value as the deferred-publication code `-99995`. For the Emmer-Compascuum small-area smoke test we pin `year=2023` (most recent populated vintage; 64 of 82 postcodes inside the AOI carry real values, the other 18 are below the 6-dwelling privacy threshold and ship `-99997` in the `nrg3:Energy/amount` slot). Future runs should bump to 2024 once CBS publishes energy values for that vintage; the encoding is unchanged, the entire vintage will simply ship `-99995` until then.

CBS publishes per-6-position-postcode aggregates derived from utility-grid connection registers. The values are **postcode-area averages**, not per-building measurements: attaching them to a `bldg:Building` or `nrg3:BuildingUnit` would misrepresent the source. Energy ADE 3.0 provides a purpose-built aggregate feature, `nrg3:UrbanFunctionArea` ([`Energy_ADE_3.0_beta8.xsd:2549-2566`](../Energy_ADE-3.0beta8/xsd/Energy_ADE_3.0_beta8.xsd)), which extends `grp:CityObjectGroup` and therefore carries its own polygon geometry, `groupMember` xlinks to constituent CityObjects, source attribution via `nrg3:Metadata`, and the `nrg3:Energy` resource hook for measured aggregates. One UrbanFunctionArea is emitted per postcode polygon that intersects the build extent (boundary clip mirrors the per-building clip in [`pipeline.filter_buildings_by_boundary`](../citygml_energy/city_builder/pipeline.py)).
...
| `gemiddeldGasverbruikWoning` | `1860` (real) / `-99997` (suppressed) | ✓ | `nrg3:Energy` resource via `nrg3:resource`: `type="actual"`, `energyCarrier="naturalGas"`, `endUse="otherOrCombination"`, `operationType="demands"`, `referencePeriod="year"`, `amount` uom `m3/a`, `isAmountNormalized=true`, `normalizationParameter="dwelling"`. | Average annual natural-gas consumption per occupied private dwelling in the postcode (m³/yr, rounded to 50). Includes `stadsverwarming`-connected dwellings (district heating), which lowers the average for postcodes on a shared heat network. **CBS sentinels (`-99997` privacy-suppressed, `-99995` deferred publication, `-99999` unknown) ride into `nrg3:Energy/amount` verbatim**: the resource is emitted unconditionally so the consumer sees the literal CBS row; only an outright `null` from the WFS skips emission. Negative amounts are therefore non-measurements; the area's `nrg3:Metadata/qualityDescription` documents the encoding. |
| `gemiddeldElektriciteitsverbruikWoning` | `2790` (real) / `-99997` (suppressed) | ✓ | second `nrg3:Energy` resource on the same UrbanFunctionArea: `type="actual"`, `energyCarrier="electricity"`, `endUse="otherOrCombination"`, `amount` uom `kWh/a`, otherwise as above. | Same sentinel-passthrough semantics. Individual connections only; excludes self-generated electricity (CBS's example: solar panels) and excludes collective consumption (lifts, gallery lighting). |
| `aantalWoningen` | `87` | ✓ | `gen:intAttribute name="dwellingCount"` on the UrbanFunctionArea | Total dwellings registered in BAG for the postcode. Carried as a generic attribute because Energy ADE 3.0 has no first-class slot for "dwelling count" on UrbanFunctionArea; the gen:intAttribute substitution group on the inherited `core:AbstractCityObject` is the schema-honest place. Sourced from BAG via CBS, so it is **not** subject to the 6-dwelling privacy suppression that gates the energy figures: a postcode with suppressed energy still carries its dwelling count. |
...
|---|---|---|
| `nrg3:UrbanFunctionArea/type` | `"postalCode6"` with codeSpace `CS_NRG3_URBAN_FUNCTION_AREA_TYPE` (project-internal vocabulary URL pinned in [`namespaces.py`](../citygml_energy/namespaces.py)) | Energy ADE 3.0 declares `UrbanFunctionArea/type` as an open `gml:CodeType` (no upstream codelist), so the publishing application supplies the vocabulary. The single value emitted today is `"postalCode6"`; future extensions (CBS buurt / wijk / vierkant) would land as additional members of the same vocabulary. |
...
The BGT classification is stored in open `gml:CodeType` slots, each `@codeSpace` naming its vocabulary (codelists are open string vocabularies, not enumerations: [`namespaces`](../citygml_energy/namespaces.py)):
...
|---|---|
| `gml:Envelope` on the `CityModel` | union of every emitted vertex: per-Pand building LoD vertices restricted to the LoDs the user requested via `config.lods` (`_collect_coordinates` in [`pand_executor.py`](../citygml_energy/city_builder/pand_executor.py); LoD0 footprint Z is mirrored from 3DBAG's nominal 0.001 m to `b3_h_maaiveld` to match the build-time lift in the building builder), every projected solar-panel vertex (`_collect_solar_panel_coordinates`), every BAG VBO address point at Z=0 (`_collect_address_coordinates`; BAG `geometriePunt` is 2D, the multiPoint builder pads Z to 0.0), and every tree crown vertex (CFTree). Written last so the envelope bounds everything that went into the file. Restricting building vertices to the requested LoDs prevents an `lods=[0]` run from claiming the LoD2.2 mesh's roof-tip Z extent. |
...
The pipeline emits the tokens below, each cross-checked against the KITModelViewer's bundled UoM list ([`KITModelViewer_V7.5.2_Build-3777/Data/UOMList.xml`](../KITModelViewer_V7.5.2_Build-3777/Data/UOMList.xml)):
...
| `kWh/m2/a` | `nrg3:Energy/amount` on the four NTA 8800 EP-Online resources, `nrg3:EnergyPerformanceCertificate/value` on NTA 8800 certs | `UOM name="KILOWATT_PER_SQUAREMETER_PER_YEAR" id="kWh/m2/a"` | primary id ✓ |
| `kg/m2/a` | `nrg3:Energy/co2Equivalent` on the BENG-2 resource for NTA 8800 rows | `UOM name="KILOGRAM_PER_SQUAREMETER_PER_YEAR" id="kg/m2/a"` | primary id ✓ |
| `MJ/a` | `nrg3:Energy/amount` on the single legacy `primary` resource for legacy_total rows, `nrg3:EnergyPerformanceCertificate/value` on legacy_total certs | `UOM name="MEGAJOULE_PER_YEAR" id="MJ/a"`. Required by the regime asymmetry: legacy methods report an absolute MJ/yr total, not a per-m² intensity. | primary id ✓ |
| `kg/a` | `nrg3:Energy/co2Equivalent` on the legacy `primary` resource for the Nader Voorschrift / ISSO branch | `UOM name="KILOGRAM_PER_YEAR" id="kg/a"`. Companion to `MJ/a`. | primary id ✓ |
| `m3/a` | `nrg3:Energy/amount` on the CBS Postcode6 `naturalGas` resource for each `nrg3:UrbanFunctionArea` | `UOM name="CUBIC_METRE_PER_YEAR" id="m3/a"` | primary id ✓ |
| `kWh/a` | `nrg3:Energy/amount` on the CBS Postcode6 `electricity` resource for each `nrg3:UrbanFunctionArea` | `UOM name="KILOWATT_HOUR_PER_YEAR" id="kWh/a"`. Chosen over `MWh/a`, which would force a 1000× rescale of the CBS-published values. | primary id ✓ |

The six `*/a` tokens are NL convention (NTA 8800 reports BENG metrics in kWh/m²·jaar; the legacy NEN 7120 lineage reports its totals in MJ/jaar and kg/jaar; CBS Postcode6 reports per-dwelling annual averages in m³/jaar and kWh/jaar).
...
| `floorNumberFrom`, `floorNumberTo`, `numberOfRooms`, `ownerName`, `ownershipType` | (no source in this pipeline) | native `nrg3:BuildingUnit/{floorNumberFrom,floorNumberTo,numberOfRooms,ownerName,ownershipType}` (XSD lines 1426-1430) | BAG VBO carries no per-VBO floor number, room count, owner identity, or tenure category, so these stay empty on every `BuildingUnit`. Listed here so a future enrichment source (e.g. WOZ for ownership, BAG `verdieping` extensions for floor numbers) has a documented landing slot rather than a `gen:*Attribute` improvisation. |
| `occupiedBy` (`nrg3:Occupants` composition on AbstractBuildingSpace) | (no source) | `nrg3:BuildingUnit/occupiedBy` ([Energy_ADE_3.0_beta8.xsd:1400](../Energy_ADE-3.0beta8/xsd/Energy_ADE_3.0_beta8.xsd#L1400)) | No occupancy schedule, dietType, income / instruction level, heat-dissipation breakdown, or schedule are sourced. CBS micro-data could in principle fill `Occupants/numberOfOccupants` per-VBO; out of scope today. |
