...
Bare-name resolution is preserved end-to-end. The canonical owner-occupier input lists `geometry_sources` in ascending-LoD order (LoD0, LoD1, LoD2, LoD3), and a bare `installedOn` name still resolves to the highest LoD present, exactly as the previous flat-index implementation did. The canonical input now also exercises the object form. Its PV carries three `installedOn` entries. One is the object form `{"name": "RoofSurface_02", "lod": 2}`, pinned to the LoD2 host face; the other two are bare `RoofSurface_01` and `RoofSurface_02`, which resolve to their LoD3 representations. The build therefore emits three relations to three distinct RoofSurface gml:ids, asserted by `tests/test_reference_building.py::test_pv_has_installed_on_relations_at_both_lods_from_json`. The full test suite passes. Two object-form tests cover the new shape. `test_installed_on_object_form_pins_relation_to_specific_lod` asserts that `{"name": "RoofSurface_01", "lod": 2}` pins to the LoD2 gml:id even when LoD3 is present; `test_installed_on_object_form_rejects_unknown_lod` asserts that an object-form entry naming a LoD where the surface is not attached raises a focused `InputFileError`.
