...
4. **Apply device relations** via [`device_relations.apply_device_relations`](../citygml_energy/device_relations.py) (re-exported from [`geometry`](../citygml_energy/geometry.py) so the call site in `input_loader` reads `from .geometry import apply_device_relations`): resolve `installed_on` references against the `surface_name_index` (then fallback to the gml:id index) and emit `nrg3:CityObjectRelation` entries on each device's `related_to` slot. Unresolved references raise loudly. Stays on its own driver because its shape — iterate the JSON `{device_id: [targets]}` config and raise on missing keys — does not fit the model-walk seam used for the derived attributes below.
...
**Parent placement: Building vs BuildingUnit.** The XSD lets `nrg3:device` ride on any `core:_CityObject` (it substitutes onto `core:_GenericApplicationPropertyOfCityObject`, XSD line 1311), so Building, BuildingUnit, Zone, and any concrete CityObject all expose a `device` slot. EnergyADE's containment hierarchy is permissive here; the canonical input authoring choice is driven by the [scope-based parent placement rule](../CONTEXT.md#scope-based-parent-placement), which asks for the smallest entity whose extent fully covers the device's served scope. Applied to the canonical input:

- **`PhotovoltaicCollector` (and by analogy `SolarThermalCollector`) parents to the Building.** The panel array is physically attached to the Pand's roof, a fact about the structure rather than about who occupies it. This matches the EnergyADE Alderaan reference and is the placement the city pipeline's solar-panel matcher emits (see [`solar_panels.py::attach_solar_collectors_to_building`](../citygml_energy/city_builder/solar_panels.py)), so both pipelines produce isomorphic GML for PV. The same parent applies to any collective device whose served scope spans more than one VBO (a central boiler feeding a riser, a shared rooftop solar-thermal array, a communal EV charger). When the served set is a strict subset of the Pand's VBOs, the device still parents to the Building and emits `nrg3:relatedTo / CityObjectRelation` xlinks (relation type `serving`) to the covered BuildingUnits.
...
| `cell_type` | CodeType | `nrg3:cellType` | `CellTypeValue.xml`. The canonical input emits `unknown` because the actual cell type was not on the panel datasheet collected from the owner-occupier; emitting a defaulted `monocrystalline` would over-report. |
...
| `nominal_efficiency` | Measure | `nrg3:nominalEfficiency` | Not exercised: the per-panel datasheet was not collected from the owner-occupier. |
| `aperture_area` | Measure | `nrg3:apertureArea` | Not exercised: same reason as `nominal_efficiency`. |
...
| `access_type` | CodeType | `nrg3:accessType` | `EVChargingAccessTypeValue.xml` (`private`, `public`, etc.). Renamed from `access` in beta8 2026-05-06. |
...
Parents to a `BuildingUnit` (not a Building): occupants are a property of a dwelling unit, not the structure. Beta8 (2026-05-06) re-rooted `nrg3:Occupants` under `core:AbstractCityObject` directly (the former `nrg3:AbstractFeatureWithLifeSpan` base was removed), so `identifier` / `valid_from` / `valid_to` / `status` are list-valued ADE hooks on every CityObject. The XSD slots `nrg3:occupiedBy` (the *attachment* slot on AbstractBuildingSpace) and `nrg3:occupiedBy` substituted onto `bldg:_GenericApplicationPropertyOfAbstractBuilding` both accept this feature; the canonical input parents to BuildingUnit and the loader walks the XSD to land the occupant in `nrg3:occupiedBy` on the unit.
...
Attaches via the `nrg3:resource` substitution slot on **any** `core:AbstractCityObject` (XSD line 1366). Every Building, BuildingUnit, Zone, and Device hosts it. The canonical input exercises three patterns:
...
A device feature can declare a list of author-facing surface refs in `installed_on`. Each entry takes one of two shapes:
...
```json
{
  ...
  "installed_on": [
    "RoofSurface_01",
    {"name": "RoofSurface_02", "lod": 3}
  ],
  ...
}
```

Resolved post-build by [`device_relations.apply_device_relations`](../citygml_energy/device_relations.py). The model-wide [`surface_name_index`](../citygml_energy/core.py) is keyed by `(STEP layer name, LoD level)`, so the same layer name appearing at multiple LoDs (e.g. `RoofSurface_01` present in both LoD 2 and LoD 3 STEPs) does not silently overwrite. The resolver collapses the LoD axis differently for the two entry shapes:
...
- **Object form** like `{"name": "RoofSurface_01", "lod": 2}`: pin the relation to the exact LoD's gml:id. No fallback to the gml:id index. Use this when the bare-name default is not what you want, e.g. when the same STEP layer name refers to different physical faces at different LoDs (the canonical pattern in this pipeline, since LoD 3 routinely subdivides an LoD 2 face and authors number sub-faces fresh per LoD; see [§ 15.8](#158-device-to-surface-relations-ambiguity-along-two-axes)).

Each entry emits one `nrg3:CityObjectRelation` with `relationType=CodeType(value="installedOn", code_space=CS_NRG3_RELATION_TYPE)` and the resolved gml:id as the related object.

Unresolved refs raise `ValueError`: silent no-op would let JSON typos slip through as missing relations that nobody notices until a downstream consumer complains. Object-form entries with a missing LoD raise a focused error that lists the LoDs the name does resolve at (so the author can fix the LoD or drop the key). Devices that lack a `related_to` field (i.e. the XSD does not allow ADE relations on that type) raise loudly too.
...
**Binding-driven scope.** Every class with a `layered_construction: list[…PropertyType]` field in the bindings receives mapping coverage automatically — the discovery walk visits every dataclass instance reachable from the model root and inspects its fields ([`construction_mapping._layered_construction_list`](../citygml_energy/construction_mapping.py)). The surface-class set is therefore controlled by the XSD, not by a hand-maintained taxonomy in `construction_mapping.py`. The CityGML 2.0 thematic surfaces (`bldg:WallSurface`, `bldg:GroundSurface`, `bldg:RoofSurface`, `bldg:Window`, `bldg:Door`) all carry the field. So do the EnergyADE ZoneBoundary subclasses (`nrg3:ZoneWallSurface`, `nrg3:ZoneGroundSurface`, `nrg3:ZoneRoofSurface`, `nrg3:ZoneIntermediateFloorSurface`, etc.).

**`by_type` keys are exact XSD element names.** A `by_type` entry of `"WallSurface"` matches `bldg:WallSurface` only — *not* `nrg3:ZoneWallSurface`, which has a different XSD element name. Authors who want zone-side surfaces to carry a layered construction must add explicit `by_type` entries (`"ZoneWallSurface": "constr_external_wall"`, etc.) or use `by_id`. The canonical input deliberately maps only the `bldg:*` surfaces and leaves the `nrg3:Zone*Surface` children without a construction xlink, because zone surfaces are an internal thermal-envelope representation that mirrors the thermal-envelope view of the same physical face — duplicating the construction xlink there is redundant. (A consequence: the `Thickness` and `HeatCapacity` per-surface attributes computed in [§ 12](#12-boundary_attributes-per-surface-descriptors) are emitted on the `bldg:*` thematic surfaces but **not** on the zone-side `nrg3:Zone*Surface` children, since the latter carry no construction. See [§ 12.1](#121-per-surface-attributes-boundarysurface).)
...
**Zone-side surfaces emit a strict subset.** `nrg3:ZoneWallSurface` / `ZoneGroundSurface` / `ZoneRoofSurface` (and the other `Zone*Surface` subclasses) carry the same `bdg_bdry_surf_*` fields as their `bldg:*` counterparts, so the discovery walk in [§ 12.4](#124-discovery-mechanism) reaches them too. They emit `TotalSurfaceArea`, `OpaqueSurfaceArea` (when openings are present), `Inclination`, and `Azimuth`. They do **not** emit `Thickness` or `HeatCapacity` in the canonical-input shape, because the canonical `construction_mapping.by_type` keys (`WallSurface`, `GroundSurface`, …) only match the `bldg:*` element names, leaving the zone-side surfaces without a `layeredConstruction` xlink and therefore without a layer stack to sum over. To populate `Thickness` / `HeatCapacity` on zone surfaces too, add explicit `by_type` entries (`"ZoneWallSurface": "constr_external_wall"`, etc.) — but consider whether the duplication is wanted: the building-thematic surface at the same physical face already carries the same data, and an aggregator that sums over both will double-count the thermal mass.

**XSD slots not computed by this pipeline.** `nrg3:bdgBdrySurfIsShared` (party-wall flag) and `nrg3:bdgBdrySurfAdditionalThermalBridgeUValue` (junction-correction U-value) are defined on `bldg:_BoundarySurface` in the EnergyADE 3.0 XSD but the boundary-attributes computation does **not** populate them. They can be authored inline on the BoundarySurface JSON dict (the schema-agnostic loader will accept them), but no implicit derivation runs: the canonical input leaves them unset because the reference building is free-standing (no shared walls) and no thermal-bridge survey was conducted. See [§ 15 gap #1](#15-schema-gap-analysis-per-building-specific) for the XSD context. Similarly, the four sky / ground view-factor slots (`bdgBdrySurfGroundViewFactor`, `bdgBdrySurfSkyViewFactor`, `bdgOpnGroundViewFactor`, `bdgOpnSkyViewFactor`) are XSD-defined but not computed by the pipeline.
...
| Areas | `m2` | 3 dp |
| Lengths | `m` | 3 dp |
| Angles | `deg` (project convention; FZK UOMList — [iai.kit.edu/uomList](https://www.iai.kit.edu/uomList) — also publishes `grad` as the German-language token for the same unit, but every emitted file in this project uses `deg` for cross-language readability) | 2 dp |
...
| `step-zonepart-lod{2,3}` | `target_zone_part_id` | `nrg3:lod{2,3}Solid` (aggregate hull) **plus** `nrg3:zoneBoundary` children (one `nrg3:Zone…Surface` per STEP layer name; openings punched into walls as `bldg:Window` / `bldg:Door`). | The classifier maps building-style STEP layer names to their `nrg3:Zone…Surface` equivalents (single source of truth: [`geometry._BLDG_TO_ZONE_SURFACE_NAME_REMAP`](../citygml_energy/geometry.py)): `WallSurface_*` → `ZoneWallSurface`, `GroundSurface_*` → `ZoneGroundSurface`, `RoofSurface_*` → `ZoneRoofSurface`, `CeilingSurface_*` → `ZoneOuterCeilingSurface`, `FloorSurface_*` → `ZoneOuterFloorSurface`, `IntermediateFloorSurface_*` → `ZoneIntermediateFloorSurface`, `ClosureSurface_*` → `ZoneClosureSurface`. Openings (`Window_*`, `Door_*`) attach to their STEP `\|parent=…` wall as `bldg:opening` children of the matched ZoneWallSurface. The aggregate Solid is still emitted for viewers that want a closed hull; the per-face children carry the `bdgBdrySurf*` thermal-envelope attributes for analysis consumers. **Inter-floor slabs** that sit fully indoors (e.g. the slab between an upstairs and a downstairs ZonePart of the same heated envelope) belong to `ZoneIntermediateFloorSurface`, not to either `Zone…Floor` or `Zone…Ceiling` *Outer* class — those Outer subclasses are reserved for surfaces whose other side faces ambient air or unheated space. |
| `step-building-lod4` | `target_building_id` | Same shape as `step-building-lod3` but on the `lod4MultiSurface` slot. | Registered in [`geometry.GEOMETRY_SOURCE_SPECS`](../citygml_energy/geometry.py) for completeness; not exercised by the canonical input (LoD 4 = full interior detail; out of scope for the renovation-passport use case). The aggregate `lod4Solid` is **not** emitted (same shape as LoD 2/3); only thematic `bldg:boundedBy` children with `lod4MultiSurface`. **`target_pv_id` is *not* accepted on this source type** — the registry's PV-target slot is wired only for LoD 2 and LoD 3 ([`geometry.py`](../citygml_energy/geometry.py): `if lod in (2, 3): target_fields["target_pv_id"] = _PV_TARGET`). LoD 4 PV geometry would need a separate registry entry. |

STEP layer names attached under a **building source** (`step-building-lod{2,3,4}`) are registered in `model.surface_name_index[layer_name] = surface.gml_id`; this is what makes `installed_on: ["RoofSurface_01"]` work. ZonePart-source faces (`step-zonepart-lod{2,3}`) are deliberately **not** indexed (`register_surface_name_index=False` in [`geometry._apply_zonepart_boundary_surfaces`](../citygml_energy/geometry.py)) — zonepart faces are an internal thermal-envelope description, not the publicly-attachable surface vocabulary that devices physically sit on. Indexing them under the same STEP layer names would silently overwrite the building-source entries (the canonical input has `RoofSurface_01` shells in *both* the LoD 3 building export and the upstairs ZonePart export). A roof-mounted PV is therefore `installedOn` the building's `bldg:RoofSurface`, never on a ZonePart's `nrg3:ZoneRoofSurface`. Author-facing layer names are not duplicated into the GML (the `gml:id` is what gets written); the index only exists in memory for the duration of the build.
...
- **`installed_on` resolution**: the `surface_name_index` is keyed by `(STEP layer name, LoD level)`, so the same layer name appearing at multiple LoDs is preserved end-to-end. A bare-name reference in JSON (e.g. `"RoofSurface_01"`) resolves to the highest LoD present for that name. The order of entries in `geometry_sources` therefore does not affect resolution. Authors who need to target a specific LoD (e.g. when the same layer name refers to different physical faces at different LoDs, which is the dominant pattern, see [§ 15.8](#158-device-to-surface-relations-ambiguity-along-two-axes)) use the explicit `{"name": "RoofSurface_01", "lod": 2}` object form documented in [§ 10](#10-installed_on-device-to-surface-relations).
...
| **Address (`core:Address`)** | Authored as a separate top-level feature parented to the Building; emitted under `bldg:address` (CityGML 2.0 composition slot, `building.xsd` line 78). Each `nrg3:BuildingUnit` xlink-references its own address via `nrg3:address/@xlink:href` (Energy ADE 3.0 UML tags `BuildingUnit.address` as `relationType=association`, XSD line 1520-1526 — i.e. a pointer, not a composition). The canonical input includes one address (Julianalaan 134, 2628 BZ Delft — used as a stand-in for the actual owner-occupier address, which is held privately). See § 2 for the JSON shape and Appendix A for the required-fields cross-check. | Same ownership pattern: one `core:Address` per BAG VBO authored once at Building level under `bldg:address`, each `nrg3:BuildingUnit` xlink-references its own address. Always populated from BAG VBO with full Dutch xAL structure. |
...
8. **Device-to-surface relations are ambiguous along two axes when the same physical face is modelled at multiple LoDs.** See [§ 15.8](#158-device-to-surface-relations-ambiguity-along-two-axes) for the worked-example writeup; the gap is part schema, part modelling discipline, and it surfaces every time a device touches a face that has been re-modelled at higher detail.

9. **No production-side member in the energy-end-use codelist.** `nrg3:Energy` requires `endUse` (XSD line 2138), but `EnergyEndUseValue.xml` enumerates only consumption end-uses (`spaceHeating`, `domesticHotWater`, `lighting`, `electricalAppliances`, `mobility`, `otherOrCombination`, and a handful of others). For a `produces`-typed resource (PV electricity, battery discharge, locally generated district heat being exported), no member describes where the produced energy goes. The canonical PV emits the off-codelist value `gridFeedIn` under `@codeSpace=EnergyEndUseValue.xml`; this is schema-valid because `gml:CodeType` is open, and matches the international standard term for renewable-energy export (EU / IEA *feed-in*, German *Netzeinspeisung*, Dutch *teruglevering*). The schema-side fix is to add production-oriented members (`gridFeedIn`, `selfConsumption`, `storage`) to the codelist, or to relax the `endUse` requirement on `produces`-typed resources.

The first four gaps are the most thesis-relevant for the renovation-passport use case (constructions, scenarios, lifecycle dates). Gap 8 is the most thesis-relevant for the multi-LoD-authoring use case, because it identifies a place where the CityGML 2.0 data model (and, as the literature confirms, CityGML 3.0 too) does not cleanly express the relation between a feature and a thematic surface that has been subdivided at higher LoD. Gap 9 is the smallest schema-side fix in the list and the only one the canonical input already works around via an off-codelist value. The remaining gaps (view factors, in-situ R-values, lifecycle dating, layer variants) are research-grade and would benefit any thermal-comfort or LCA work that builds on top of EnergyADE 3.0.

---

### 15.8. Device-to-surface relations: ambiguity along two axes

The `installed_on` field on every device feature becomes one or more `nrg3:CityObjectRelation` entries in the output, each pointing at one surface gml:id with `relationType="installedOn"`. The canonical input exercises the field on the PV array, which sits on the building's roof:

```json
"installed_on": ["RoofSurface_01", "RoofSurface_02"]
```

Two distinct problems sit underneath this innocuous-looking field. The canonical input exercises both at once.

#### Vertical ambiguity (across levels of detail)

The reference building is authored at both LoD 2 and LoD 3. The PV carries geometry at both levels, with one aggregated polygon at LoD 2 and thirty-six individual panel polygons at LoD 3. The building's roof is also represented at both levels, with two large faces at LoD 2 and eight subdivided sub-faces at LoD 3. Two of the LoD 3 sub-faces happen to be labelled `RoofSurface_01` and `RoofSurface_02` in the LoD 3 STEP file, and the LoD 2 faces happen to use those same labels in the LoD 2 STEP file. The names are not preserved-identity across LoDs; the author numbers sub-faces fresh per LoD, and the same label may refer to a different physical face at LoD 2 than at LoD 3.

An `installed_on` entry is just a string. The author writes `"RoofSurface_01"` and expects the link to resolve. The data model has no native concept of LoD on a relation, so the resulting `CityObjectRelation` can only point at one specific surface gml:id, and each LoD's roof has its own gml:id. The relation is structurally LoD-aware (every gml:id implicitly carries the LoD of the geometry attached to it), but the authoring layer is not. Before this fix, the in-memory `surface_name_index` was a flat `{name: gml:id}` dictionary; when two STEP files registered the same name at different LoDs, the last write won, and `installed_on` resolved to whichever LoD happened to be imported last (ascending-LoD ordering of `geometry_sources` was the unspoken convention). The pipeline silently dropped any LoD-specific link that was not the last write, and a downstream consumer working at the dropped LoD got nothing back from a roof that physically carried the device.

The fix at the pipeline level is to make the index LoD-aware and let the author choose what to do with the LoD axis. The index is now keyed by `(name, LoD)`. A bare-name entry like `"RoofSurface_01"` resolves to the highest LoD present for the name; this matches the dominant consumer intent (the most detailed available representation of the physical face the device sits on) and closes the order-dependent silent failure. An object-form entry `{"name": "RoofSurface_01", "lod": 2}` pins the relation to a specific LoD's representation. When the same label refers to different physical faces at different LoDs, which is the dominant pattern in this pipeline because LoD 3 routinely subdivides, the bare-name fan-out across LoDs is unsafe: the LoD 2 face labelled `RoofSurface_01` may not be the same physical thing as the LoD 3 face labelled `RoofSurface_01`, and emitting a relation to both would create false data. The object form is therefore the schema-honest shape when the author needs LoD-specific relations.

A deeper modelling question sits above the fix. The data model assumes one feature per physical face, with multiple LoD geometries colocated on that feature (Kutzner et al. 2020 confirms the same intent in CityGML 3.0, where `Space` and `SpaceBoundary` subclasses can carry multi-LoD geometries simultaneously). The per-(face, LoD) shape this pipeline emits ([§ 13.1](#131-per-lod-distinct-boundarysurfaces-and-the-modelling-rules-question), deliberate, justified by LoD 3 subdivision) breaks that assumption, and the relation system inherits the breakage. Neither CityGML 2.0 nor CityGML 3.0 cleanly resolves what to do when LoD 3 subdivides an LoD 2 face into multiple smaller features; the standards assume the 1:1 feature-to-LoD mapping that real building data does not always allow.

#### Horizontal ambiguity (across faces at one level of detail)

The canonical PV lists two roof faces because the panels physically cross the ridge. Some panels sit on one face, some on the other. The relation captures that the PV touches both faces, but cannot say how much of the array is on each. A consumer that wants per-face apportionment, for shading analysis or for splitting energy yield across orientations, has no native field to read.

The data model preserves the geometric truth. The PV carries its own `lod{2,3}MultiSurface`, and the roof faces carry theirs. A consumer can derive the apportionment by intersecting them. What is missing is a quantitative slot on the relation itself. `nrg3:CityObjectRelation` carries a qualitative `relationType` (drawn from `OtherRelationTypeValue.xml`, with members `installedOn`, `connectedTo`, `serving`) and an xlink to the related object, and that is all. The schema cannot say "70 % of this PV is installed on roof A and 30 % on roof B."

A future Energy ADE revision could add an `installationCoverage` measure on the relation, in the range [0, 1], so the canonical two-face PV would emit two relations with coverage 0.7 and 0.3. Today the workaround for an author who needs the apportionment is to model the array as N separate PV features, one per face, each with proportional `installed_power` and `number_of_devices`, all pointing at a master PV that aggregates the energy production resource. The convention is verbose, has no schema-level signal that the splitting is a workaround, and forces every consumer to learn the master-and-parts pattern. It is acceptable for one-off documentation, not for a stock-scale pipeline.

#### Why this matters

The two axes both come from the same root. The relation system in Energy ADE 3.0 assumes one feature per physical thing, with multi-LoD geometry colocated on that feature and qualitative point-to-point links between features. Two patterns that are normal in real building data break the assumption. A device is installed on a roof whose LoD 2 and LoD 3 representations are separate features. A device's footprint covers part of multiple roof faces. The pipeline can patch the first by emitting LoD-aware relations through the `(name, lod)` index and the explicit object form; the second needs a schema extension and stays as the workaround above until then.

References. CityGML 3.0 versioning concepts (the `identifier` attribute stable across versions, the `Version` and `VersionTransition` classes) are documented in Kutzner et al. (2020) and address identity across **time**; the corresponding concept for identity across **LoDs** is not in the standard. The Scenario ADE draft (Agugiaro 2026, Beta 6) targets renovation-state branching and not multi-LoD identity. The Alderaan reference dataset shipped with Energy ADE 3.0 beta 8 does not exercise multi-LoD per face for any device, so it sets no precedent for either axis.
...
4. **WTW balanced heat-recovery ventilation system.** The arch. spec states 'Ventilatievoorzieningen: mechanische toevoer en afvoer d.m.v. een gebalanceerd WTW systeem op basis van CO2-sturing.' EnergyADE 3.0 provides `nrg3:AirDistribution` (mechanical ventilation device) and heat-recovery efficiency attributes, but no `nrg3:MechanicalVentilation` feature has been authored in the canonical input. To model: add an `nrg3:AirDistribution` feature parented to the BuildingUnit (analogue of `nrg3:ThermalDistribution`, which serves the unit; see § 5 Devices for the Building-vs-BuildingUnit convention), capturing the system type, heat-recovery efficiency (not in the spec extract — would need the unit's datasheet), and CO2-controlled operation mode.
