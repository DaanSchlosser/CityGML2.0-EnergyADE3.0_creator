...
**Parent placement: Building vs BuildingUnit.** The XSD lets `nrg3:device` ride on any `core:_CityObject` (it substitutes onto `core:_GenericApplicationPropertyOfCityObject`, XSD line 1311), so Building, BuildingUnit, Zone, and any concrete CityObject all expose a `device` slot. EnergyADE's containment hierarchy is permissive here; the canonical input authoring choice is therefore driven by physical semantics rather than schema constraint. The convention this pipeline follows:

- **`PhotovoltaicCollector` (and by analogy `SolarThermalCollector`) parents to the Building.** The panel array is physically attached to the Pand's roof — a fact about the structure, not about who occupies it. This also matches the EnergyADE Alderaan reference and is the placement the city pipeline's PV matcher emits (see [`pv_panels.py::attach_pv_collectors_to_building`](../citygml_energy/city_builder/pv_panels.py)), so both pipelines produce isomorphic GML for PV.
- **Every other device parents to the BuildingUnit.** Heat pump, thermal distribution, thermal storage, boiler (if exercised), EV charger, and the per-unit electrical appliances all serve and meter the single occupied unit. For a single-VBO Pand, this lands them under the lone `nrg3:BuildingUnit`; for a multi-unit Pand, each apartment would carry its own device set.
...
- **`installed_on` resolution**: the `surface_name_index` is keyed by STEP layer name (e.g. `"RoofSurface_01"`). When the same layer name appears in both the LoD 2 and the LoD 3 STEP **and** the LoD 3 source is listed *after* the LoD 2 source in `geometry_sources`, the LoD 3 entry overwrites the LoD 2 entry (last write wins); device-to-surface xlinks therefore resolve to the LoD 3 face. The canonical input lists `step-building-lod0` → `lod1` → `lod2` → `lod3` in that order, which is the convention authors should follow: the last-write-wins behaviour is *order-dependent*, not a property of the LoD level. Listing LoD 3 before LoD 2 would resolve `installed_on` against the LoD 2 face instead. The validator does not enforce this ordering today; treat ascending-LoD ordering as the authoring convention until it does.
