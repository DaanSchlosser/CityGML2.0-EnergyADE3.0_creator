...
| `thermal_conductivity` | Measure | `nrg3:thermalConductivity` | uom `W/(K*m)`. The canonical input bracket-encloses the denominator (`W/(K*m)`, `J/(K*kg)`) on per-material thermal properties for SI-conformant readability; the older bracket-free `W/K*m` shape is still schema-permissible (no UOMList entry pins the bracket placement) but kept for the legacy `u_value` / `r_value` slots below to match the FZK UOMList tokens (`UVALUE` / `THERMAL_RESISTANCE` rows). |
