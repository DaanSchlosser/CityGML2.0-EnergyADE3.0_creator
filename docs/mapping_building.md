...
5. **Apply construction mapping** via [`construction_mapping.apply_construction_mapping`](../citygml_energy/construction_mapping.py) (also re-exported through `geometry`): iterate every dataclass that has a `layered_construction` list field; resolve a construction id via `by_id` (gml:id keyed) or `by_type` (XSD-element-name keyed); append an `xlink:href` reference using the binding-resolved property-type wrapper class.
6. **Attach boundary attributes** via [`boundary_attributes.attach_boundary_surface_attributes`](../citygml_energy/boundary_attributes.py): compute and write up to six `bdgBdrySurf*` attributes per BoundarySurface (`TotalSurfaceArea`, `OpaqueSurfaceArea`, `Inclination`, `Azimuth`, `Thickness`, `HeatCapacity`) and three `bdgOpn*` per Opening from the geometry and the layered-construction xlinks set in step 5.
...
| `address` (resolved via a child `core:Address` feature with `parent: <building_unit_id>`) | xlink to `core:Address` | `core:Address` (xAL-flavoured, NL convention), attached on the BuildingUnit's `nrg3:address` slot | The canonical input authors a **separate top-level feature** of `type: "core:Address"` with `parent` set to the BuildingUnit `id`; the loader's two-phase build attaches it to the BuildingUnit's address slot. The address dict carries `xal_address.address_details.country.{country_name_code, country_name, locality.{locality_name, thoroughfare.{thoroughfare_number, thoroughfare_name}, postal_code.postal_code_number}}`, mirroring the xAL element tree. The full canonical-input shape lives in [`inputs/buildings/owner_occupier_building.json`](../inputs/buildings/owner_occupier_building.json) and the city pipeline's [`builders/address.py`](../citygml_energy/city_builder/builders/address.py) is the single source of truth on the xAL conventions (Locality/@Type=`Town`, Thoroughfare/@Type=`Street`, ISO 3166-1 alpha-2 country code, BAG-canonical hyphen separator for `huisnummertoevoeging`). The per-building pipeline does not auto-translate flat fields — author the nested form directly. Inline (non-feature) authoring of an address dict on the BuildingUnit's own `address` field is also schema-permissible but not the convention used here. |
...
| **Address (`core:Address`)** | Authored on the BuildingUnit when collected (matches BAG VBO scope). The canonical input includes one address (Julianalaan 134, 2628 BZ Delft — used as a stand-in for the actual owner-occupier address, which is held privately). See § 2 for the JSON shape and Appendix A for the required-fields cross-check. | Always populated from BAG VBO with full Dutch xAL structure. |
...
| `core:Address` (inside `nrg3:BuildingUnit.address[*].address`) | `xal_address` (REQUIRED on `AddressType`). The xAL substructure is itself nested but has no required leaves at the AddressDetails level — every xAL element below `AddressDetails` is `minOccurs="0"`, so the schema accepts an `<AddressDetails/>` carrying nothing. The canonical input populates `country.country_name_code` + `country.country_name` + `country.locality.{locality_name, thoroughfare.{thoroughfare_number, thoroughfare_name}, postal_code.postal_code_number}` to match the city pipeline's BAG-derived shape. |
