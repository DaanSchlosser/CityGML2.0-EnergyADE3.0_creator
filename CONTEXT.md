...
**VBO**:
*Verblijfsobject*, the BAG legal usable-area unit inside a Pand (one dwelling, one shop, one office suite, one garage). It carries one address, occasionally without a postcode (as for a garage or storage box), and at most one EP-Online certificate. Mapped one-to-one to an `nrg3:BuildingUnit`, regardless of how complete its address is.
_Avoid_: unit, dwelling, apartment, occupant (use VBO when speaking BAG-natively); assuming every VBO address carries a postcode.
...
**Address-key VBO**:
The subset of VBOs whose BAG record carries both a postcode and a huisnummer, so it can take part in the EP-Online *address-key* fallback match. That is the only thing the predicate gates. It is **not** a precondition for emitting a unit: every VBO becomes one `nrg3:BuildingUnit`, identified by its `verblijfsobject_id` (the BAG identificatie, always present, emitted as an `nrg3:identifier` and carried in the `bu_<id>` gml:id), and whatever address parts exist are emitted (a partial `bldg:address`, or none for a garage or storage box that has no postcode). The `verblijfsobject_id` is the primary EP-Online match key (the `BAGVerblijfsobjectID` column); the `(postcode, huisnummer, huisletter, toevoeging)` address key is only the fallback for older labels that predate the BAG-id enrichment. The predicate lives in [`address_match`](citygml_energy/city_builder/address_match.py).
_Avoid_: dropping a VBO that lacks a postcode or huisnummer (it is still a BuildingUnit with a `verblijfsobject_id`); treating "matchable" as a gate on emitting a unit or on the id match, rather than only on the address-key fallback.
...
**EP-Online**:
The Dutch national register of energy-performance certificates. The Mutatiebestand ships one row per certificate, keyed primarily on `BAGVerblijfsobjectID`, so every certificate describes one VBO and parents to the `nrg3:BuildingUnit`; when a VBO has several over time, the most recent wins. Under NTA 8800 the figure is `final` energy in kWh/m²·yr; the older regimes (NEN 7120, ISSO 82.3) report a primary-energy total in MJ/yr, so the regime changes the units and the energy numbers, not where the certificate sits. See [`mapping_city.md` §6](docs/mapping_city.md) for the regime-aware mapping.
...
The same ordering applies to non-device features by analogy. An EPC parents to whichever scope the document was issued for. An EP-Online certificate is per VBO for every regime (NTA 8800 and the older NL methods alike), so it always parents to the BuildingUnit. A certificate genuinely issued for a whole building, such as a hand-authored German EnEV one, would parent to the `bldg:Building`; the XSD and the generic per-building input both allow it, though no current input emits one. An `nrg3:Energy` resource parents to whichever scope the measurement covers: a per-VBO smart-meter reading → BuildingUnit; a whole-Pand utility-meter reading → Building.
