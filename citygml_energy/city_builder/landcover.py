"""Semantic landcover step: the 3D Basisvoorziening ground behind one seam.

This module owns the whole landcover concern the way
...
landcover is *semantic*: instead of a bare-earth relief it emits the Dutch
...
standard library, so unlike the previous AHN raster path the landcover step
needs no numpy / GeoTIFF stack. **No environment keys.** The PDOK endpoints
are public, so nothing is read from ``.env``; the ``landcover`` config block is
a knob-less opt-in (its presence enables semantic landcover).

Like the other enrichment steps the whole concern soft-fails: a PDOK outage,
an AOI outside coverage, or a corrupt tile degrades to a landcover-free build
...
class LandcoverSource:
    """Validated ``landcover`` config block: a knob-less opt-in marker.

    The 3D Basisvoorziening ground carries every landcover class at a fixed LoD
    from a single national product, so there is nothing to tune: a present
    ``landcover`` block means "fetch the semantic landcover", an absent one means
...
def fetch_landcover(
    ...
    *clip_to_box* mirrors the building clip so the two layers share one rule: a
    ...
    city build continues without semantic landcover. A magic-valid archive whose
    ...
    _LOG.info("Landcover: parsed %d 3DBV object(s) inside the AOI", len(objects))
    return objects
...
def attach_landcover_to_model(
    ...
    *,
    ...
    grows to cover the ground without the coordinate sink swelling by every
    surface vertex.
    """
    if not objects:
        return 0
    ...
    # (which imports LandcoverSource) without dragging the xsdata bindings into
    ...
    _LOG.info("Landcover: attached %d feature(s)", count)
    return count
