...
* LoD 2 → ``boundedBy`` with ``bldg:GroundSurface`` / ``bldg:WallSurface``
  / ``bldg:RoofSurface`` elements, each carrying its own
  ``bldg:lod2MultiSurface``. Polygons without a recognised semantic
  type fall back to ``WallSurface``.
...
index for buildings lives in ``docs/source_to_gml_mapping.md``.
...
from ...gml_builders import build_multi_surface, build_solid
...
from ._common import UOM_AREA_M2, UOM_METRES, UOM_PERCENT, UOM_VOLUME_M3, inner_type
...
def _lift_lod0_to_ground(
    ...
    """
    target_z = to_float(parsed.attributes.get("b3_h_maaiveld"))
    if target_z is None:
...
def _apply_building_attributes(building: Any, attrs: dict[str, Any]) -> None:
    ...
    * ``b3_h_dak_max - b3_h_maaiveld`` → ``bldg:measuredHeight``: total
      physical height of the building in metres. ``measuredHeight``
    ...
    * ``b3_volume_lod22`` → ``nrg3:area``-style ``QualifiedVolume``
    ...
    if h_dak is not None and h_maaiveld is not None and h_dak > h_maaiveld:
        building.measured_height = LengthType(
            value=round(h_dak - h_maaiveld, 3), uom=UOM_METRES,
        )
